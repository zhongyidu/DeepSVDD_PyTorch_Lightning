...
class CIFAR10_LeNet_ELU(nn.Module):
    ...
    def forward(self, x):
        x = self.pool(F.elu(self.bn2d1(self.conv1(x))))
        x = self.pool(F.elu(self.bn2d2(self.conv2(x))))
        x = self.pool(F.elu(self.bn2d3(self.conv3(x))))
        x = x.view(x.size(0), -1)
        x = self.fc1(x)
        return x
...
class CIFAR10_LeNet_ELU_Autoencoder(nn.Module):
    def __init__(self):
        super(CIFAR10_LeNet_ELU_Autoencoder, self).__init__()
        self.logger = logging.getLogger(self.__class__.__name__)
        self.rep_dim = 128
        self.pool = nn.MaxPool2d(2, 2)
        # Encoder
